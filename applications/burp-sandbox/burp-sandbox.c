...
void onCommand(void *handler_arg, esp_event_base_t base, int32_t id, void *event_data);
...
void start(void) {
    ...
    ESP_ERROR_CHECK(burpCommandInit(GPIO_NUM_0, GPIO_NUM_3));

    ESP_ERROR_CHECK(esp_event_handler_register(COMMAND_EVENT, ESP_EVENT_ANY_ID, onCommand, NULL));
